...
<div id="page">
	...
	<div id="content">
		...
		<td>ID</td>
		<td>Data</td>
		<td>Uzytkownik, ktory wykonał operacje</td>
		<td>Uzytkownik, który zobaczył operacje</td>
		<td>Operacja</td>
		</tr>
		
		<% 
		int key = 1;
		for (UserLog currentLog : employeeManager.getAllUserLogs() ){ %>
		<tr <% if(key % 2 == 0){ %> style="background-color: #777777"<%}else{ %> style="background-color: #B2B2B2"<% } %> >
		<td><%=key++ %></td>
		<td><%=currentLog.getUloDate() %></td>
		<td><%=currentLog.getUserWhoAdd().getUsrName() + " " + currentLog.getUserWhoAdd().getUsrSurname() %></td>
		<td><%=currentLog.getUserWhoViewd().getUsrName() + " " + currentLog.getUserWhoViewd().getUsrSurname() %></td>
		<td><%=currentLog.getActionDone().getAdoName() %></td>
		</tr>
